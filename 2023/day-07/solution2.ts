...
const calculateStrength = (input: string) => {
  ...
  let pairCount = [...counts.values()].reduce((prev, curr) => {
    if (curr === 2) {
      return prev + 1;
    }
    return prev;
  ...
  const hasWild = counts.has("J");
  if (hasWild) {
    // we have a wild
    const wildCount = counts.get("J");
    // we don't want to count the wild in our pair counts
    if (wildCount === 2) {
      pairCount = pairCount - 1;
    }
    // all ways to make 5 of a kind
    if (fourOfAKind && wildCount === 1) {
      return 7;
    }
    if (threeOfAKind && wildCount === 2) {
      return 7;
    }
    if (pair && wildCount === 3) {
      return 7;
    ...
    // all ways to make 4 of a kind
    if (threeOfAKind && wildCount === 1) {
      return 6;
    }
    if (pair && wildCount === 2) {
      return 6;
    ...
    if (pairCount === 2 && wildCount === 1) {
      return 5;
    }
    if (pairCount === 1 && wildCount === 2) {
      return 5;
    }

    // ways to make three of a kind
    if (pairCount === 1 && wildCount === 1) {
      return 4;
    }
    if (pairCount === 0 && wildCount == 2) {
      return 4;
    }

    // ways to make 2 pair of two: N/A

    // ways to make a pair of two
    if (pairCount === 0 && wildCount == 1) {
      return 2;
    }
  }
...
run();
//
// console.log(calculateStrength("234JJ"));
