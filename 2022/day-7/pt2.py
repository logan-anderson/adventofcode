...
total = get_total_size(tree.root, accum=allDirs)

DISK_SIZE = 70000000
SPACED_NEEDED_FOR_UPDATE = 30000000
USED = total
UNUSED = DISK_SIZE - USED
SPACED_NEEDED = SPACED_NEEDED_FOR_UPDATE - UNUSED


dirsCanBeDeleted = []
for dirNode in allDirs:
    if dirNode[1] > SPACED_NEEDED:
        dirsCanBeDeleted.append(dirNode)

smallestDir = sorted(dirsCanBeDeleted, key=lambda x: x[1])[0]
print(smallestDir)
